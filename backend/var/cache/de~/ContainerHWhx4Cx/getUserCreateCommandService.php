<?php

namespace ContainerHWhx4Cx;

use Symfony\Component\DependencyInjection\Argument\RewindableGenerator;
use Symfony\Component\DependencyInjection\ContainerInterface;
use Symfony\Component\DependencyInjection\Exception\RuntimeException;

/**
 * @internal This class has been auto-generated by the Symfony Dependency Injection Component.
 */
class getUserCreateCommandService extends App_KernelDevDebugContainer
{
    /**
     * Gets the private 'App\Command\UserCreateCommand' shared autowired service.
     *
     * @return \App\Command\UserCreateCommand
     */
    public static function do($container, $lazyLoad = true)
    {
        include_once \dirname(__DIR__, 4).'/vendor/symfony/console/Command/SignalableCommandInterface.php';
        include_once \dirname(__DIR__, 4).'/vendor/symfony/console/Command/Command.php';
        include_once \dirname(__DIR__, 4).'/src/Command/UserCreateCommand.php';

        $container->privates['App\\Command\\UserCreateCommand'] = $instance = new \App\Command\UserCreateCommand();

        $instance->setName('app:user:create');
        $instance->setDescription('Add a short description for your command');

        return $instance;
    }
}
